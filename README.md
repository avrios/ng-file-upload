# Avrios Fork of ng-file-upload

## Fix process

* Branch from most recent release tag:

```
git checkout tags/TAG_NAME
git checkout -b avr/fix/BRANCH_NAME
```

* Change and fix the issue. Commit only /src/ changes (not dist files)
* Create a PR against `avr/develop`, and also the upstream if the fix could be useful to others
* When the PR is approved and rebased onto `avr/develop`, also rebuild and commit the `dist` files

## Pulling in upstream changes

Checkout our fork and add the upstream as a remote:

```
git clone git@github.com:Avrios/ng-file-upload.git
git remote add upstream https://github.com/danialfarid/ng-file-upload
```

Merge upstream's master into ours:
...
Then update the dependencies in our main app
******************************************************************
******************************************************************
